...
int OnInit()
{
  ...
  customIndicatorHandle = iCustom(_Symbol, PERIOD_H2, "Market\\TrendCatcher.ex5");

  ATR_Handle = iATR(_Symbol, PERIOD_M10, RSI_PERIOD);
  lot_size = SymbolInfoDouble(_Symbol, SYMBOL_VOLUME_MIN);
  if ( ATR_Handle == INVALID_HANDLE || customIndicatorHandle == INVALID_HANDLE)
  {
...

{
 ...
 // double irsiv = iRSIBuffer[0];
  ...
  //if (rsi_previous > RSI_OVERBOUGHT &&  StringFind(_Symbol, CRASH_SYMBOL) != -1 && current_ask < spikePrice && downTrend[0] != 1.7976931348623157e+308)
  ...
  //if (rsi_previous < RSI_OVERSOLD && StringFind(_Symbol, BOOM_SYMBOL) != -1 && current_bid > spikePrice && upTrend[0] != 1.7976931348623157e+308)
