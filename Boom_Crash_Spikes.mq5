...
input double stop_loss_multiplier = 0.3;   // Stop loss multiplier
input double take_profit_multiplier = 1.0; // Take profit multiplier
double base_lot = 0;
...
void OnTick()
{
    ...
    else
    {
        ...
        if (StringFind(_Symbol, BOOM_SYMBOL) != -1 && upTrendArrow[0] != 1.7976931348623157e+308){
        ...
        {   
            ...
            //double stop_loss = current_tick - (STOP_LOSS * symbolPoint);
            int digits = (int)SymbolInfoInteger(_Symbol, SYMBOL_DIGITS);
            double pip2point = MathPow(10, digits - 4); 
            double stop_loss = current_tick - STOP_LOSS * pip2point;
            double take_profit = current_tick + TAKE_PROFIT * pip2point;
    ...
    else if (StringFind(_Symbol, CRASH_SYMBOL) != -1 && downTrendArrow[0] != 1.7976931348623157e+308)
    {
